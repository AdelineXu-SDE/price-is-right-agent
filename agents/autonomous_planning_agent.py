...
class AutonomousPlanningAgent(Agent):
    ...
    def __init__(self, collection):
        """Coordinates scanning, pricing, and notification into one controlled workflow."""
    ...
    def scan_the_internet_for_bargains(self) -> str:
        """Tool: scan the internet for deals (dedup via memory)."""
    ...
    def estimate_true_value(self, description: str) -> str:
        """Tool: estimate true value via ensemble pricing."""
    ...
    def notify_user_of_deal(
        ...
        """Tool: notify user once about the best deal."""
        if self.opportunity:
            self.log(
            ...
            self.messenger.notify(description, deal_price,
                                  estimated_true_value, url)
            deal = Deal(product_description=description,
                        price=deal_price, url=url)
    ...
    def plan(self, memory: List[str] = []) -> Optional[Opportunity]:
        """Run the full workflow and return the best surfaced Opportunity (if any)."""
